...
if (!STRAPI_API_TOKEN) {
  ...
  /**
   * Sanitize text fields to handle strict validation
   */
  ...
  async function createOrGetTag(tagData) {
    try {
      ...
      const response = await api.post('/api/tags', {
        data: {
          name: sanitizeField(tagData.name, 100),
          slug: sanitizeField(tagData.slug, 100),
          description: sanitizeField(tagData.description, 500),
  ...
  async function createOrGetPost(postData, tagIdMap) {
    try {
      ...
      // Create new post with SANITIZED fields
      const response = await api.post('/api/posts', {
        data: {
          title: sanitizeField(postData.title, 200),
          slug: sanitizeField(postData.slug, 200),
          format: sanitizeField(postData.format || 'article', 50),
          content: postData.content || '', // Rich text, usually no limit
          excerpt: sanitizeField(postData.excerpt || '', 300), // Limited to 300 chars
          readingTimeMinutes: postData.readingTimeMinutes || null,
          durationSeconds: postData.durationSeconds || null,
          youtubeUrl: sanitizeField(postData.youtubeUrl || '', 500),
          externalUrl: sanitizeField(postData.externalUrl || '', 500),
          isFeatured: postData.isFeatured || false,
          seoTitle: sanitizeField(postData.seoTitle || '', 150), // SEO title limit
          seoDescription: sanitizeField(postData.seoDescription || '', 160), // SEO description limit
          tags: tagIds,
          publishedAt: new Date().toISOString(),
        },
      });
