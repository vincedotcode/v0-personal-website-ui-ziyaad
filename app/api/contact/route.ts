...
${meetingPart}
  ...
  console.log("[contact] Sending email to Zi via Resend:", {
    to: adminEmail,
    subject,
  ...
  await resend.emails.send({
    from: `Zi Website <${CONTACT_FROM_EMAIL}>`,
    to: adminEmail,
    subject,
    text: textBody,
...
I've received your message and I'm looking forward to our meeting${
    payload.meetingDateTime ? ` on ${payload.meetingDateTime}` : ""
  }.
...
This is an automated message. Please don't reply to this email.
  ...
  console.log("[contact] Sending auto-reply to user via Resend:", {
    to: payload.email,
    from: fromEmail,
    subject,
  })
  ...
  if (!process.env.RESEND_API_KEY) {
    console.warn("[contact] RESEND_API_KEY is not set, skipping auto-reply.")
    return
  }

  await resend.emails.send({
    from: `Zi Website <${fromEmail}>`,
    to: payload.email,
    subject,
    text: textBody,
  })
